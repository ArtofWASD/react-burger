...
const FeedOrderInfo: FC = () => {
  ...
  const filtredIngredients = order.ingredients.map((item) => {
    const result = ingredients.filter((data: TData) => data._id === item)[0];
    ...
    <div className="grid grid-flow-row justify-center px-5">
      {order.number === orderId ? (
        <>
          ...
          <div>{filtredIngredients && filtredIngredients.map((item) => <FeedOrderInfoIngredient data={item} key={item._uniqueId} />)}</div>
